...
[{1 :VonHeikemen/lsp-zero.nvim
  ...
  :config
  (fn [_ opts]
    (let [lsp (require :lsp-zero) cmp (require :cmp)]
      ...
      ((. (require :mason-null-ls) :setup)
       {:ensure_installed wanted-servers :automatic_installation false :handlers []})
      ...
      (let [cmp-actions (lsp.cmp_action)]
        (cmp.setup
          {
            ...
            :mapping
            {
              ...
              "<C-f>" (cmp-actions.luasnip_jump_forward)
              "<C-b>" (cmp-actions.luasnip_jump_backward)}
            :sources
            (cmp.config.sources
              [
