...
#define BUILD_INFORMATION "local build on " __DATE__ " at " __TIME__
#endif
...
enum {
  ...
  MACRO_ANY,
  MACRO_TOGGLE_QUKEYS,
  MACRO_MOVETO_BASE_LAYER,
  MACRO_SWITCHTO_QWERTY,
  MACRO_SWITCHTO_DVORAK,
...
static uint8_t baseLayerIndex = 0;

static void moveToBaseLayer(KeyEvent &event) {
  if (!keyToggledOn(event.state)) {
    return;
  }

  Layer.move(baseLayerIndex);
}

static void switchToQwerty(KeyEvent &event) {
  if (!keyToggledOn(event.state)) {
    return;
  }
...
static void switchToDvorak(KeyEvent &event) {
  if (!keyToggledOn(event.state)) {
    return;
  }
...
static void switchToColemakDh(KeyEvent &event) {
  if (!keyToggledOn(event.state)) {
    return;
  }
...
const macro_t *macroAction(uint8_t macro_id, KeyEvent &event) {
  ...
  case MACRO_MOVETO_BASE_LAYER:
    moveToBaseLayer(event);
    break;
  case MACRO_SWITCHTO_QWERTY:
    switchToQwerty(event);
    break;
  case MACRO_SWITCHTO_DVORAK:
    switchToDvorak(event);
    break;
  case MACRO_SWITCHTO_COLEMAK_DH:
    switchToColemakDh(event);
    break;
  }
  return MACRO_NONE;
}
...
void setup() {
  // QUKEYS(
  //   kaleidoscope::plugin::Qukey(kaleidoscope::plugin::Qukeys::layer_wildcard, KeyAddr(3, 7), LSHIFT(Key_9)),
  //   kaleidoscope::plugin::Qukey(kaleidoscope::plugin::Qukeys::layer_wildcard, KeyAddr(3, 8), LSHIFT(Key_0))
  // )
  ...
  const uint8_t EEPROM_LAYER_COUNT = 10;
  ...
  EEPROMKeymap.setup(EEPROM_LAYER_COUNT);

  // We need to tell the Colormap plugin how many layers we want to have custom
  // maps for. To make things simple, we set it to eight layers, which is how
  // many editable layers we have (see above).
  ColormapEffect.max_layers(EEPROM_LAYER_COUNT);
  ...
  LayerNames.reserve_storage(17 * EEPROM_LAYER_COUNT);
